...
export default function ChatWindow({ userId }: ChatWindowProps) {
  ...
  const extractOrderDetails = (text: string) => {
    ...
    const phoneMatch = text.match(/(?:want|need|buy|order|get).*(?:phone|iphone|samsung|pixel)/i);
    const simMatch = text.match(/(?:want|need|buy|order|get).*(?:sim|esim|card)/i);
    const internetMatch = text.match(/(?:want|need|buy|order|get).*(?:internet|wifi|broadband)/i);
    const tvMatch = text.match(/(?:want|need|buy|order|get).*(?:tv|television)/i);
  ...
  const needsOrderConfirmation = (text: string) => {
    return text.includes("To confirm your order") && text.includes("please reply with");
  };
              ...
              {messages.map((msg, idx) => (
                ...
                >
                  <div
                    ...
                    } ${msg.isOrderConfirmation ? 'border-green-500 border-l-4' : ''} ${hasInvoiceContent(msg.text) && msg.isBot ? 'invoice-content' : ''}`}
                  >
                    ...
                    {msg.isBot && needsOrderConfirmation(msg.text) && (
                      <div className="mt-3 grid grid-cols-2 gap-2">
                        ...
                        {(() => {
                          const productMatch = msg.text.match(/order for (\w+)/i);
                          const planMatch = msg.text.match(/with the (\w+) plan/i);
                    ...
                    {msg.isBot && hasOrderRequest(msg.text) && !needsOrderConfirmation(msg.text) && (
                      <div className="mt-3">
                        <div className="text-xs text-gray-500 mb-2">Quick Order Options:</div>
                        <div className="grid grid-cols-2 gap-2">
                          <Button
                            variant="outline"
                            size="sm"
                            className="bg-blue-50 text-blue-600 hover:bg-blue-100 border border-blue-200"
                            onClick={() => handleQuickOrder("SIM", "Unlimited")}
                          >
                          ...
                          <Button
                            ...
                            size="sm"
                            className="bg-blue-50 text-blue-600 hover:bg-blue-100 border border-blue-200"
                            onClick={() => handleQuickOrder("Internet", "Premium")}
                          >
                            <ShoppingCart size={14} className="mr-2" />
                            Internet Premium
                          </Button>
