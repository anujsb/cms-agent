...
function detectOrderIntent(message: string): { isOrderIntent: boolean; productName?: string; plan?: string } {
  ...
  // Extract product info if it's an order intent
  if (hasOrderIntent) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (message.toLowerCase().includes("confirm order") && message.toLowerCase().includes("yes")) {
      try {
        // Extract product name and plan from the message or session
        // For this example, we'll use placeholder values if not detected
